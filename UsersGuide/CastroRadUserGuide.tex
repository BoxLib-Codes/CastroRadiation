...
\item Castro allows for two temperatures (different radiation and gas
  ...
  $\kappa_F$, and Rosseland mean, $\kappa_R$, opacities---these have
  different weightings.
  
  If we set $\kappa_F \Delta x \gg 1$ ($\kappa_F$ is really large),
  then the two temperatures become the same.

  If we set $\kappa_F = \kappa_R$, then we can see how different the
  two temperature are.
